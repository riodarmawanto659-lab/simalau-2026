...
class Pesanan extends Model
{
    ...
    {
        // Pengingat pengambilan tidak boleh dibuat saat pesanan baru dibuat,
        // karena pada tahap itu tanggal_siap_diambil masih kosong.
        // Sesuai BRD/PRD, pengingat baru dibuat setelah cucian berstatus
        // Siap Diambil dan belum diambil minimal 3 hari.
        if ($this->status_pesanan !== 'siap_diambil' || ! $this->tanggal_siap_diambil) {
            return;
        }

        if ($this->tanggal_siap_diambil->gt(now()->subDays(3))) {
            return;
        }
        ...
        $this->pengingatPengambilan()->updateOrCreate(
            ...
            [
                ...
                'jumlah_hari_tertahan' => (int) max($this->tanggal_siap_diambil->diffInDays(now()), 0),
                'status_pengingat' => $this->pengingatPengambilan?->status_pengingat ?: 'aktif',
                'catatan' => $this->pengingatPengambilan?->catatan ?: 'Pengingat otomatis dibuat karena cucian belum diambil minimal 3 hari setelah siap diambil.',
            ]
        );
    }
    ...
    {
        if ($this->status_pesanan === 'siap_diambil') {
            $this->ensureContactReminder();
